    ...
    int connexion_generic(char* sname, int port) {
    #if defined (WIN32)
        WSADATA WSAData;
        ...
        if (a.debug) {
            char buf[150];
            sprintf(buf, "Vous êtes connecté au serveur %s\n", sname);
            debug(buf);
        }
    ...
    int connexion(char* sname) {
        return connexion_generic(sname, PORT);
    }

    int connexion_local() {
        return connexion_generic("localhost", LOCALPORT);
    }
    ...
    {
        char buffer[MAXREP];
        envoyer_recevoir(message, buffer);
    }
